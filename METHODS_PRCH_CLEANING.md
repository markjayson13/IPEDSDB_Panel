...
The repo keeps every institution-year row, but blanks component values identified by PRCH metadata as parent-reported instead of leaving duplicated values on the child row.

The method preserves the institution-year spine and cleans the duplicated component payload.
...
- a cleaned wide panel should still preserve row-level institutional history while preventing duplicate component-family counts

Policy: preserve the panel spine and clean the duplicated payload.

The method is narrow: clean what the metadata and PRCH flags support, and leave the rest visible in QA rather than guessing.

## Literature and source support

### 1. `Using IPEDS for Panel Analyses: Core Concepts, Data Challenges and Empirical Applications`

Local file:

- `$IPEDSDB_ROOT/Paneling guide/Using IPEDS for Panel Analyses Core Concepts Data Challenges and Empirical Applications.pdf`
...
- `$IPEDSDB_ROOT/Paneling guide/Conducting research with IPEDS.pdf`
...
- `$IPEDSDB_ROOT/Paneling guide/IPEDS Data Collection System Glossary.pdf`
...
- `$IPEDSDB_ROOT/Checks/wide_qc/qc_column_lineage.csv`

The dictionary artifacts are authoritative for the exact `PRCH_*` labels present in the Access-database build used by this repo. The column-lineage artifact is authoritative for mapping final wide-panel columns back to the source families that Stage 06 carried into the wide build. Stage 07 consumes that lineage when it decides which columns each `PRCH_*` flag can null.
...
- `Checks/prch_qc/prch_clean_columns.csv` records the exact columns targeted by each flag and their lineage source families
- `Checks/prch_qc/prch_lineage_summary.csv` records whether cleaning used Stage 06 column lineage or a dictionary fallback
...
For Finance, the expected post-clean result is:
...
Do not interpret the cleaned panel as a universal institutional collapse to parent level. It is a row-preserving, component-aware cleaning layer, not a full organizational consolidation.

In papers, memos, or handoffs, describe it as a cleaned institution-year panel, not a single "true" consolidated institution history.
...
- Local supporting files under `$IPEDSDB_ROOT/Paneling guide/`, including:
